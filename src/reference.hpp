...
namespace reference
{
	...
	{
		return reference::natural_3way_compare(left, right) < 0;
	}
...
// string_views are not guaranteed to be null terminated
// from_chars uses an output variable while returning a pair to signal error condition, interesting design choice
// You cannot add to an iterator behind the end of the container, e.g. for checking a condition even if you never dereference it (at least in msvc debug it throws an exception)
// Never dereference iterators if you dont have to, especially end iterators -> I am dumb.
// from_chars is not constexpr sadly, prevents my experiment of constexpr ing the algorithm

// I wanted to use std::strong_ordering from <compare> at first instead of an int as return value, but I had some problems with the return values,
// so I concluded that maybe its not meant for external use, anyhow I did not get it to work.
